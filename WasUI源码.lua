...
function WasUI:LoadLucide()
    ...
    -- v3 module: 1756 icons embedded as base64, each cached individually
    -- Primary: jsDelivr CDN (fast), Fallback: GitHub raw
    local moduleUrls = {
    ...
    -- Fallback: latte-soft v0.363.0 (1476 icons, uses sprite sheets)
    if not content or content == "" then
        for _ = 1, 2 do
            local ok, result = pcall(game.HttpGet, game, "https://github.com/latte-soft/lucide-roblox/releases/download/0.1.3/lucide-roblox.luau")
            if ok and result and #result > 5000 then
                content = result
                break
            end
    ...
    if not ok or not module then
        warn("[WasUI] Lucide 模块执行失败")
        return nil
    end
    if type(module) ~= "table" or type(module.GetAsset) ~= "function" then
        warn("[WasUI] Lucide 模块格式无效")
        return nil
    end
...
function WasUI:GetIcon(iconName)
    ...
    if lucide then
        ...
        if ok and icon and icon.Url then return icon end
    end
    return nil
end
